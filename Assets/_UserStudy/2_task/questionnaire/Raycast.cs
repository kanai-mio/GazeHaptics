...
public class Raycast : MonoBehaviour
{
    ...
    private int[] answers = new int[] { 0, 0, 0 , 0, 0};
    ...
    private static string fileName = $"Questionnaire_ID{userID}_times{Times}_term{termNo}";
    private string filePath = @"C:\Users\mio\Desktop\userstudy\Questionnaire\" + fileName + ".csv";
    void CreateCSV()
    {
        //CSVファイルにanswersを出力
        using (StreamWriter writer = new StreamWriter(filePath))
        {
            ...
            {
                writer.WriteLine($"Q{i + 1},{answers[i]}");
            }
        }

        Debug.Log($"CSVファイルが生成されました: {filePath}");

        Debug.Log($"answers: {answers[0]}, {answers[1]}, {answers[2]}");
    }
