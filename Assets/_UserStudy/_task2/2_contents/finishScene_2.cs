...
public class finishScene_2 : MonoBehaviour
{
    public class EyeGazeData
    {
        ...
        // コンストラクタ
        public EyeGazeData(float time, int hitObject, float hitPosition_x, float hitPosition_y, float hitPosition_z)
        {
            Time = time;
            HitObject = hitObject;
    ...
    void CreateCSV()
    {
        ...
        using (StreamWriter writer = new StreamWriter(filePath))
        {
            ...
            {
                writer.WriteLine($"{data.Time},{data.HitObject},{data.HitPosition_x},{data.HitPosition_y},{data.HitPosition_z}");
            }
        }
    ...
    void Update()
    {
        ...
        Vector3 hitPos = abatar_gazehaptics.instance.hitPos;
        EyeGazeData newData = new EyeGazeData(currentTime, objectID, hitPos.x, hitPos.y, hitPos.z);
        ...
        {
            CreateCSV();
            SceneManager.LoadScene("Questionnaire_2");
        }
    }
}
