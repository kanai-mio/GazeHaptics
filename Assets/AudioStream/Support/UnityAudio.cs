...
namespace AudioStreamSupport
{
    ...
    public static class UnityAudio
    {
        ...
        public static float[] ResampleFrame(float[] sourceFrame, int sourceRate, int targetRate)
        {
            ...
            {
                double t = i / (double)resampledLength;
                var originalIndex = t * sourceFrame.Length;

                int index1 = Mathf.FloorToInt((float)originalIndex);
                index1 = Mathf.Min(index1, sourceFrame.Length - 1);
                int index2 = Mathf.Min(index1 + 1, sourceFrame.Length - 1);

                float fraction = (float)originalIndex - index1;
