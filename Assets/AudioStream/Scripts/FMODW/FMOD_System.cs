    ...
    public static partial class FMOD_SystemW
    {
            ...
            )
        {
            ...
            if (FMOD_SystemW.systems4devices.TryGetValue(forOutputDriver, out var refc_system))
            {
                ...
                Log.LOG(LogLevel.INFO, logLevel, gameObjectName, "Retrieved FMOD system {0} for output: {1} [new refc {2}]", system.FMOD_System.system.handle, forOutputDriver, system.refcount);
            ...
            else
            {
                ...
                Log.LOG(LogLevel.INFO, logLevel, gameObjectName, "Created FMOD system {0} for output: {1} [refc {2}]", system.system.handle, forOutputDriver, 1);
            ...
            )
        {
            ...
            if (FMOD_SystemW.systems4devices.TryGetValue(driverId, out var refc_system))
            {
                ...
                //    Debug.LogWarningFormat("System being released was not previously created via manager");
                ...
                if (--refc_system.refcount < 1)
                {
                    ...
                    Log.LOG(LogLevel.INFO, logLevel, gameObjectName, "Released system {0} for output: {1} [refc {2}]", fmodsystem.system.handle, driverId, refc_system.refcount);
                ...
                else
                {
                    ...
                    Log.LOG(LogLevel.INFO, logLevel, gameObjectName, "Decreased refcount on system {0} for output: {1} [new refc {2}]", fmodsystem.system.handle, driverId, refc_system.refcount);
                }
            }
            else
            {
                ...
                if (fmodsystem != null)
                {
                    ...
                    Log.LOG(LogLevel.INFO, logLevel, gameObjectName, "Released system {0}, for output: {1}", fmodsystem.system.handle, driverId);
        ...
        public partial class FMOD_System
        {
            ...
            /// </summary>
            public readonly OUTPUT_DEVICE_INFO initialOutpuDevice;
                ...
                )
            {
                ...
                */
                ...
                int od_namelen = 255;
                result = this.system.getDriverInfo(forOutputDriver, out var od_name, od_namelen, out var od_guid, out var od_systemrate, out var od_speakermode, out var od_speakermodechannels);
                ...
                this.initialOutpuDevice = new OUTPUT_DEVICE_INFO()
                {
                    id = forOutputDriver,
                    name = od_name,
                    guid = od_guid,
                    samplerate_driver = od_systemrate,
                    speakermode_driver = od_speakermode,
                    channels_driver = od_speakermodechannels
                };
            }
            /// <summary>
        ...
        static FMOD.RESULT DEBUG_CALLBACK(FMOD.DEBUG_FLAGS flags, IntPtr file, int line, IntPtr func, IntPtr message)
        {
            ...
            string message_s = new FMOD.StringWrapper(message);
            if (!message_s.Contains("FMOD_RESULT = 63")) // missing tags in stream is reported for every frame
            {
                string file_s = new FMOD.StringWrapper(file);
                string func_s = new FMOD.StringWrapper(func);
